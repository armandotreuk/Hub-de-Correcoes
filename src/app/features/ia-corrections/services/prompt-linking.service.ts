...
export class PromptLinkingService {
  ...
  private courses: Course[] = [
    ...
    { id: 101, name: 'Engenharia de Software', clusterId: 1, clusterName: 'Cluster Sul', businessUnitId: 1 },
    ...
    { id: 201, name: 'Ciência da Computação', clusterId: 2, clusterName: 'Cluster Norte', businessUnitId: 1 },
    { id: 202, name: 'Gestão de RH', clusterId: 2, clusterName: 'Cluster Norte', businessUnitId: 1 },
    { id: 203, name: 'Contabilidade', clusterId: 2, clusterName: 'Cluster Norte', businessUnitId: 1 },
    // Cluster Centro — Unicesumar
    { id: 301, name: 'Engenharia Civil', clusterId: 3, clusterName: 'Cluster Centro', businessUnitId: 2 },
    { id: 302, name: 'Psicologia', clusterId: 3, clusterName: 'Cluster Centro', businessUnitId: 2 },
    { id: 303, name: 'Medicina Veterinária', clusterId: 3, clusterName: 'Cluster Centro', businessUnitId: 2 },
    // Cluster Oeste — Unicesumar
    { id: 401, name: 'Arquitetura e Urbanismo', clusterId: 4, clusterName: 'Cluster Oeste', businessUnitId: 2 },
    { id: 402, name: 'Design Gráfico', clusterId: 4, clusterName: 'Cluster Oeste', businessUnitId: 2 },
    { id: 403, name: 'Marketing Digital', clusterId: 4, clusterName: 'Cluster Oeste', businessUnitId: 2 },
  ];
  ...
  linkPromptToCourse(promptId: string, promptTitle: string, courseId: number, activityTypeName: string): Observable<PromptLink> {
    // Validação RN07: unicidade (Curso + Tipo de Atividade)
    const exists = this.links.find(l => l.courseId === courseId && l.activityTypeName === activityTypeName);
    if (exists) {
  ...
  updateCoursePrompt(linkId: string, newPromptId: string, newPromptTitle: string): Observable<PromptLink> {
    const index = this.links.findIndex(l => l.id === linkId);
    if (index >= 0) {
      this.links[index] = { ...this.links[index], promptId: newPromptId, promptTitle: newPromptTitle };
    }
